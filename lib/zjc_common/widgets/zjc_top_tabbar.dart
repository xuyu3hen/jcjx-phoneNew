...
import '/zjc_common/base_appbar.dart';
import 'zjc_badge.dart';
...
class ZjcTopTabBarModel {
  final String? title; // 标题
  final Widget? widget; // 对应的widget
  final ZjcBadge? badge; // 设置badge，不用设置title

  const ZjcTopTabBarModel({this.title, this.widget, this.badge});
}

class ZjcTopTabBar extends StatefulWidget {
  const ZjcTopTabBar({
    Key? key,
    this.title = '',
  ...
  final String title;
  final List<ZjcTopTabBarModel> tabModelArr; // tab对象数组
  ...
  @override
  State<ZjcTopTabBar> createState() => _ZjcTopTabBarState();
}

class _ZjcTopTabBarState extends State<ZjcTopTabBar> with SingleTickerProviderStateMixin {
...
//          _tabController.indexIsChanging     监听不到滑动
  ...
  Widget build(BuildContext context) {
    ...
    // var labelColor = KColors.dynamicColor(context, provider.getThemeColor(), KColors.kThemeColor);
        var indicatorColor = KColors.dynamicColor(context, Theme.of(context).primaryColor, KColors.kThemeColor);
    var labelColor = KColors.dynamicColor(context, Theme.of(context).primaryColor, KColors.kThemeColor);
