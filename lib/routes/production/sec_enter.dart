...
class _SecEnter extends State<SecEnter>{
              ...
              ZjcFormSelectCell(
                ...
                rightWidget: 
                  MultiSelectChipDisplay(
                    items: repairTimesSelected.map((e) => MultiSelectItem(e,e.name!)).toList(),
                    scroll: true,
                ...
                clickCallBack: () async {
                  if(repairProc["name"] == ""){
                    ...
                    final _items = repairTimesList
                      .map((vehicle) => MultiSelectItem<RepairTimes>(vehicle, vehicle.name!))
                      .toList();
                    await showDialog(
                      context: context,
                      builder: (ctx){
  ...
  Future<String> newEntry() async {
    ...
    var r = await ProductApi().newTrainEntry(      
    ...
    return SafeArea(
      child: InkWell(
        ...
        child: Container(
          ...
          height: 50,
          child: const Text('新 增 入 段', style: TextStyle(color: Colors.black,fontSize: 18)),
        ),
      )
    );
  }
      ...
      builder: (con){
        return Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              Text("新增入段提报成功",style: TextStyle(fontSize: 18),),
              ConstrainedBox(
