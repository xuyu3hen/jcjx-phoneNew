...
class UnqualifiedList extends StatefulWidget {
  ...
  const UnqualifiedList(this.trainEntryCode,{Key? key}) : super(key: key);

    @override
  State<UnqualifiedList> createState() => _UnqualifiedListState();
}

class _UnqualifiedListState extends State<UnqualifiedList> {
  // 列表尽头
   ...
   void _queryEntryData() async{
    var data = await JtApi().getJtList(
      queryParameters: {
        ...
        'pageNum':pageNum,
        'pageSize':10,
        'repairPersonnel': Global.profile.permissions?.user.userId,
  ...
  Widget _itemWidget(JtMessage item) {
    var cell = Padding(
      ...
      child: Container(
        ...
        child: InkWell(
          onTap: () {
            Navigator.of(context).pushNamed('repair',arguments: item).then((value) => {
              search()
            });
          },
          child: Column(
