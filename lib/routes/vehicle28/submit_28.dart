...
class _Vehicle28FormState extends State<Vehicle28Form>{
                ...
                clickCallBack: () {
                  ZjcCascadeTreePicker.show(context,
                    ...
                    clickCallBack: (selectItem, selectArr) {
                      print(selectArr);
                      setState((){
                        requiredProcessingMethod["dictName"] = selectItem["dictName"];
                        requiredProcessingMethod["code"] = selectItem["code"];
                      });
                    },
                  );
                },
      ...
      child: InkWell(
        onTap: () async {
          if((faultDesc == null||
              ...
              riskLevel == null||requiredProcessingMethod['code'] == ""
              )
          ){
            showToast("内容未填写完");
          }else{
            ...
            try{
              ...
              if(completeStatus == 1){
                ...
                queryParameters["repairPersonnel"] = Global.profile.permissions?.user.userId;
              }
              if(faultPics.isNotEmpty){
                await JtApi().uploadJt(imagedata: faultPics[0]).then((value) async => {
                  if(value['data'] != null&&value['data'] != ""){
                    queryParameters["repairPicture"] = value['data'],
                    l.insert(0, queryParameters),
                    submit = await JtApi().uploadJt28(
                      queryParametrs: l
                    ),
                    if(submit['data'] != null){
                    ...
                    builder: (con){
                      return Container(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            Text("机统28提报成功",style: TextStyle(fontSize: 18),),
                            ConstrainedBox(
        ...
        child: Container(
          ...
          height: 50,
          child: const Text('故障提报', style: TextStyle(color: Colors.black,fontSize: 18)),
        ),
      )
    );
  }
  ...
  Future<String> selectDate(str)async{
    ...
    if(val != null&&str == "datetime"){
      String str = formatDate(val, [yyyy, '-', mm, '-', dd,' ',HH, ':', nn, ':', ss]);
      print(str);
      return str;
