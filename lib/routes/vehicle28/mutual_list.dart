...
class MutualList extends StatefulWidget {
  const MutualList({super.key});

    @override
  State<MutualList> createState() => _MutualListState();
}
   ...
   void _queryEntryData() async{
    var data = await JtApi().getJtList(
      queryParameters: {
        ...
        'pageNum':pageNum,
        'pageSize':10,
        'mutualInspectionPersonnel': Global.profile.permissions?.user.userId,
      }
    );
  ...
  Widget _itemWidget(JtMessage item) {
    var cell = Padding(
      ...
      child: Container(
        ...
        child:InkWell(
          onTap: () {
            Navigator.of(context).pushNamed('mutual',arguments: item).then((value) => {
              search()
            });
          },
          child: Column(
